...
export function AnomalyChart({ invoices }) {
  const { data, insight } = useMemo(() => {
    ...
    const generatedInsight = anomalyDetected
      ? `A spike in spending was detected on ${anomalyDate} compared to the normal average of $${mean.toFixed(2)}.`
      : `No significant spending anomalies detected. Your spending is relatively stable. (Average: $${mean.toFixed(2)})`;
